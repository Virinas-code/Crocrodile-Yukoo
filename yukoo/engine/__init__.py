...
class EngineBase:
    ...
    def search(self, board, depth, maximize_white, limit_time):
        ...
        eval, move = self.minimax_yukoo(board, depth, maximize_white, limit_time)
    ...
    def yukoo_select_best_moves(self, board: chess.Board):
    ...
    def minimax_yukoo(self, board: chess.Board, depth, maximimize_white, limit_time):
        """Minimax algorithm from Wikipedia with Yukoo algorithm."""
        self.nodes += 1
        if depth == 0 or board.is_game_over():
            evaluation = self.evaluate(board)
            # evaluation = self.evaluate(board)
        ...
        if maximimize_white:
            ...
            legal_moves = list(board.legal_moves)
            list_best_moves = [legal_moves[0]]
            for move in self.yukoo_select_best_moves(board):
                if time.time() > limit_time:
                    return float("inf"), chess.Move.from_uci("0000")
                test_board = chess.Board(fen=board.fen())
                test_board.push(move)
